...
if config.dset:find('103_large') then
   -- This is tuned for 2 node 4 * K80 with 2-way ipcGroup and SOC between IPC groups
   -- We are mostly limited by PCIe BW but this is an interesting experiment
   -- nvidia-smi topo -m
   ...
   -- Even if we use NCCL, it is still advantageous to group by IPC domains and let
   -- the custom allreduce deal with cross IPC domains.
   mpi.start{ withCuda = usegpu,
              withIPCGroups = true,
              withCartesianCommunicator = false }

   -- Collective configuration for the 103_large dataset
   mpi.C.torchmpi_set_collective_num_threads(4)
   mpi.C.torchmpi_set_num_buffers_per_gpu_collective(8)
   mpi.C.torchmpi_set_max_buffer_size_per_gpu_collective(2^22)

   -- On this configuration, hierarchical always outeperforms flat
   -- mpi.C.torchmpi_set_flat_collectives()
   mpi.C.torchmpi_set_hierarchical_collectives()

   -- NCCL can be up to 25% slower in this case
   -- mpi.collectiveSelector.gpu.singlenode.allreduceTensor =
   --    mpi.nccl.allreduceTensor
   -- mpi.collectiveSelector.gpu.multinode.allreduceTensor =
   --    mpi.nccl.allreduceTensor
else
...
local nmachines = mpi.size() -- config.nmachines
local mid       = mpi.rank() + 1 -- config.machineid
...
for epoch = 1, config.maxepoch do
    ...
    rec:resetStates()
    -- Register async backward
    mpinn.async.registerAsyncMPIBackward(model)
    ...
    for i = 1, wdata:size(1) - bptt, bptt do
        ...
        local data_time = tm:time().real * 1000; tm = torch.Timer()
        ...
        cutorch.synchronize()
        mpi.barrier()
        local prop_time = tm:time().real * 1000; tm = torch.Timer()

        local comm_time = 0
        mpinn.async.synchronizeGradients(model)
        cutorch.synchronize()
        mpi.barrier()
        if not config.full_batch then
            if nchunks % config.update_freq == 0 then
                comm_time = tm:time().real * 1000; tm = torch.Timer()
        ...
        cutorch.synchronize()
        mpi.barrier()
        local optim_time = tm:time().real * 1000; tm = torch.Timer()
    ...
    -- Unregister async backward (prepare for saving)
    mpinn.async.unregisterAsyncMPIBackward(model)
